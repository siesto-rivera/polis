...
export async function handle_POST_delphi_jobs(req: Request, res: Response): Promise<void> {
  try {
    logger.info(`Creating Delphi job with parameters: ${JSON.stringify(req.body)}`);
    ...
    if ((!report_id && !conversation_id)) {
      res.status(400).json({
        status: 'error',
        error: 'Missing required parameter: either report_id or conversation_id must be provided',
      });
      return;
    }
    ...
    const zid = conversation_id || (report_id ? await getConversationIdFromReportId(report_id) : null);
    ...
    try {
      logger.info(`Putting job item in DynamoDB: ${JSON.stringify({
      ...
      logger.error(`Error writing to DynamoDB: ${dbError instanceof Error ? dbError.message : dbError}`);
      throw dbError; // Let the outer catch handle it
    }
    
  } catch (error) {
    logger.error(`Error creating Delphi job: ${error instanceof Error ? error.message : error}`);
...
async function getConversationIdFromReportId(report_id: string): Promise<string | null> {
  try {
    ...
    logger.error(`Error mapping report_id to conversation_id: ${error instanceof Error ? error.message : error}`);
