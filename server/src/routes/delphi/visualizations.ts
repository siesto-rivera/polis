 ...
 */
export async function handle_GET_delphi_visualizations(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`Fetching visualizations for report_id: ${report_id}, conversation_id: ${conversation_id}`);
    ...
    try {
      ...
      logger.info(`Listing S3 objects with params: ${JSON.stringify(listObjectsParams)}`);

      try {
        s3Response = await s3Client.send(new ListObjectsV2Command(listObjectsParams));
        
        // Log successful response
        logger.info(`S3 listing successful. Found ${s3Response.Contents?.length || 0} objects.`);
    ...
    for (const obj of s3Response.Contents) {
      ...
      if (!fileName.endsWith('.html') && !fileName.endsWith('.png') && !fileName.endsWith('.svg')) {
        continue;
      }
      ...
      try {
        ...
        logger.error(`Error generating signed URL for ${key}: ${err.message || err}`);
    ...
    const jobsWithVisualizations = Object.keys(visualizationsByJob).map(jobId => {
 ...
 */
async function fetchJobMetadata(report_id: string, conversation_id: string): Promise<Record<string, any>> {
  try {
    // Configure DynamoDB client
    const dynamoDBConfig: any = {
      region: process.env.AWS_REGION || "us-east-1",
      endpoint: "http://dynamodb:8000", // Local DynamoDB
      credentials: {
